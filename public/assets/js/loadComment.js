...
async function displayComments() {
    ...
    if (data.length > 0) {
        data.forEach((element) => {
            ...
            photoProfil.setAttribute('src','../../assets/images/dummy.webp')
            photoProfil.setAttribute('alt','placeHolder for profil picture')
            photoProfil.setAttribute('loading','lazy')
            let userName = document.createElement('p')
            userName.innerText = `${element.name} - publié le : ${element.date}`
            userName.setAttribute('class','comment-user-info')
            topSection.appendChild(photoProfil)
